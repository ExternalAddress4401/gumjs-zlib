...
const mustCall = function (fn, expected) {
...
const tmpDir = path.join(testRoot, tmpDirName);
...
if (!fs[rFS]) {
  ...
  const files = {
    "elipses.txt": fs.readFileSync(
      path.resolve("test/fixtures", "elipses.txt")
    ),
    ...
    "person.jpg.gz": fs.readFileSync(
      path.resolve("test/fixtures", "person.jpg.gz")
    ),
    "pseudo-multimember-gzip.gz": fs.readFileSync(
      path.resolve("test/fixtures", "pseudo-multimember-gzip.gz")
    ),
    "pseudo-multimember-gzip.z": fs.readFileSync(
      path.resolve("test/fixtures", "pseudo-multimember-gzip.z")
    ),
  };
  Object.keys(files).forEach((file) => {
    files[path.resolve(fixturesDir, file)] = files[file];
  });
...
export { fixturesDir, mustCall, tmpDirName, tmpDir, refreshTmpDir };
